...
pub async fn forge_updates(configuration: &configuration::Configuration, id_pipe_slug: &str, neoforge: Option<&str>) -> Result<models::ForgeUpdates, Error<ForgeUpdatesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id_pipe_slug = id_pipe_slug;
    let p_query_neoforge = neoforge;

    let uri_str = format!("{}/updates/{id_slug}/forge_updates.json", configuration.base_path, id_slug=crate::apis::urlencode(p_path_id_pipe_slug));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ForgeUpdates`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ForgeUpdates`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn statistics(configuration: &configuration::Configuration, ) -> Result<models::Statistics, Error<StatisticsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Statistics`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Statistics`")))),
        }
    } else {
        let content = resp.text().await?;
